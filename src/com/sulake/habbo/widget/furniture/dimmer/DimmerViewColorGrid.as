package com.sulake.habbo.widget.furniture.dimmer
{
   ...
   public class DimmerViewColorGrid
   {
      ...
      private var var_480:BitmapData;
      
      private var var_1179:XML;
      
      private var var_316:IWindowContainer;
      
      private var var_393:IItemGridWindow;
      
      private var var_814:BitmapData;
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         var _loc13_:* = null;
         var _loc14_:* = null;
         var_393.destroyGridItems();
         var_316 = null;
         for each(_loc2_ in colors)
         {
            _loc3_ = param1.buildFromXML(var_1179) as IWindowContainer;
            _loc3_.addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,onClick);
            _loc3_.background = true;
            _loc3_.color = 4294967295;
            _loc3_.width = var_480.width;
            _loc3_.height = var_480.height;
            var_393.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_480.width,var_480.height,true,0);
               _loc4_.bitmap.copyPixels(var_480,var_480.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_814.width,var_814.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(var_675.width,var_675.height,true,16777215);
               _loc6_.bitmap.copyPixels(var_675,var_675.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = var_393.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_393.numGridItems)
         {
            return;
         }
         select(var_393.getGridItemAt(param1) as IWindowContainer);
      }
