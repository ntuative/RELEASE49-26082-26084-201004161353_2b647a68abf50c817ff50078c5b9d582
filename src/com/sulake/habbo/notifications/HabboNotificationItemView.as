package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements IUpdateReceiver
   {
      ...
      public static const const_663:int = 70;
      
      private static const const_779:int = 2;
      
      public static const const_418:int = 4;
      
      private static const const_780:int = 3;
      
      private static const const_408:int = 1;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         if(_window.height < var_1516)
         {
            _window.height = var_1516;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(var_31)
         {
            ...
            case const_408:
               var_871 += param1;
               _loc2_ = Number(var_871) / Number(var_418["time_fade_in"]);
               if(var_871 > int(var_418["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_779:
               var_1044 += param1;
               if(var_1044 > int(var_418["time_display"]) && !var_1272)
               {
                  startFadeOut();
               }
               break;
            case const_780:
               var_870 += param1;
               _loc3_ = 1 - Number(var_870) / Number(var_418["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_870 > int(var_418["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_199.orientation == "TOP" ? int(const_349) : 0;
         var _loc4_:int = var_199.orientation == "BOTTOM" ? int(_loc2_.height - const_349) : int(_loc2_.height);
         var _loc5_:int = var_199.orientation == "LEFT" ? int(const_349) : 0;
         var _loc6_:int = var_199.orientation == "RIGHT" ? int(_loc2_.width - const_349) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_418;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_418;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_602;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_602;
            }
         }
         else if(false)
         {
