package com.sulake.habbo.avatar.actions
{
   ...
   public class AvatarActionManager
   {
      ...
      public function AvatarActionManager(param1:IAssetLibrary, param2:String)
      {
         ...
         super();
         var_168 = param1;
         var_187 = new Dictionary();
         var_687 = param2;
         switch(param2)
         {
            case AvatarType.const_83:
               if(!var_168.hasAsset("HabboAvatarActions"))
               {
                  Logger.log("[AvatarStructure] Could not init human Action Data.");
               }
               else
               {
                  _loc3_ = var_168.getAssetByName("HabboAvatarActions").content as XML;
               }
               break;
            case AvatarType.PET:
               if(!var_168.hasAsset("HabboAvatarPetActions"))
               {
                  Logger.log("[AvatarStructure] Could not init pet Action Data.");
               }
               else
               {
                  _loc3_ = var_168.getAssetByName("HabboAvatarPetActions").content as XML;
               }
         }
         if(_loc3_ == null)
         {
      ...
      private function filterActions(param1:Array) : Array
      {
         ...
         while(_loc6_ < param1.length)
         {
            ...
            _loc5_ = var_187[_loc4_.actionType] as ActionDefinition;
            if(_loc5_ != null)
            {
      ...
      public function sortActions(param1:Array) : Array
      {
         ...
         {
            _loc4_ = var_187[_loc3_.actionType];
            if(_loc4_ != null)
            {
      ...
      public function getCanvasOffsets(param1:Array, param2:String, param3:int) : Array
      {
         ...
         while(_loc7_ < param1.length)
         {
            ...
            _loc6_ = var_187[_loc5_.actionType] as ActionDefinition;
      ...
      private function parseActionOffsets() : void
      {
         ...
         var _loc10_:* = null;
         for each(_loc2_ in var_187)
         {
            _loc3_ = _loc2_.state;
            if(var_168.hasAsset("action_offset_" + _loc3_))
            {
               _loc4_ = var_168.getAssetByName("action_offset_" + _loc3_).content as XML;
               if(_loc4_.@avatartype == var_687)
               {
