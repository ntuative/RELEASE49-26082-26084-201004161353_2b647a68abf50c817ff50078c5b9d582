package com.sulake.core.window.components
{
   ...
   public class ScrollbarLiftController extends InteractiveController implements IDragBarWindow
   {
      ...
      {
         if(param2.type == WindowNotifyEvent.const_357)
         {
            _offsetX = x != 0 ? Number(x / Number(0 - width)) : Number(0);
            var_795 = y != 0 ? Number(y / Number(0 - height)) : Number(0);
            if(var_8 != _scrollBar)
            {
               _scrollBar.update(this,new WindowNotifyEvent(WindowNotifyEvent.const_249,this,null));
            }
         }
