package com.sulake.core.runtime.events
{
   ...
   public class EventDispatcher implements IEventDispatcher, IDisposable
   {
      ...
      private static const const_1116:uint = 1;
      ...
      public function dispatchEvent(param1:Event) : Boolean
      {
         if(!_disposed)
         {
            ...
            if(_result == const_784)
            {
               if(var_730 != null)
               {
                  Core.crash("Error catched in EventDispatcher: undefined",var_730.errorID,var_730);
               }
               else
               {
                  Core.crash("Error catched in EventDispatcher. No error data available!",0,var_730);
               }
            }
            return _result == const_499;
         }
         return false;
      }
      ...
      private function eventProcessor(param1:Event) : void
      {
         ...
         var array:Array = var_272[event.type];
         if(array)
         {
            callbacks = [];
         ...
         _result = !!event.isDefaultPrevented() ? uint(const_1116) : uint(const_499);
      }
      ...
      public function dispose() : void
      {
         ...
         if(!_disposed)
         {
            for(_loc1_ in var_272)
            {
               _loc2_ = var_272[_loc1_] as Array;
               for each(_loc3_ in _loc2_)
               {
                  _loc3_.callback = null;
               }
               delete var_272[_loc1_];
            }
            var_272 = null;
